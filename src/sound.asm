...
stream_tempo        .rs 6   ; The tempo which is added to the ticker below
                            ...
                            ; is the next tick
stream_loop1        .rs 6   ; Loop counter, used for finite loop 1
stream_loop2        .rs 6   ; Loop counter, used for finite loop 2
...
.fetch
    ...
    bcc .note_length
.opcode:                    ; Else ($A0-$FF), an opcode
    jsr se_opcode_launcher  ; Handle the opcode
    iny
    lda stream_status, x    
    and #%00000001          ; If stream is still enabled, 
    bne .fetch              ; fetch the next byte
